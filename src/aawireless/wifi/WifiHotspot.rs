WifiHotspot(boost::asio::io_context &ioService, aawireless::configuration::Configuration &configuration, std::string password) : ioService(ioService),configuration(configuration),password(password) {
